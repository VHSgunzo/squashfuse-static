...
for dependency in SQUASHFUSE MIMALLOC LIBFUSE XZ LZO ZLIB LZ4 ZSTD SUPER_STRIP
do
...
if grep -E '^(SQUASHFUSE|MIMALLOC|XZ|LZO|ZLIB|LZ4|ZSTD)_(VERSION|COMMIT)=git([[:space:]]|$)' "$BUILD" >/dev/null; then
    fail 'moving git source marker remains'
fi

if grep -E 'git checkout "?v?\$(SQUASHFUSE|MIMALLOC)_VERSION' "$BUILD" >/dev/null; then
...
grep -F 'These pinning and reproducibility qualifications apply to the musl matrix only' "$ROOT/README.md" >/dev/null ||
    fail 'README does not limit source-pinning guarantees to the musl matrix'
