...
run_publish()
{
    ...
    mkdir "$state"
    : >"$log"
    marker=squashfuse-static-release-owner:test-marker-$mode
    set +e
...
grep -F 'release create --draft --verify-tag --title -v3 --notes squashfuse-static-release-owner:test-marker-missing-success -- -v3' "$RUN_LOG" >/dev/null ||
...
set +e
rerun_marker=squashfuse-static-release-owner:test-marker-rerun
