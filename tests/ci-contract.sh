...
grep -F 'name: squashfuse-${{ matrix.arch }}' "$WORKFLOW" >/dev/null ||
    fail 'per-architecture upload name is missing'
grep -F 'release/squashfuse-musl-mimalloc-${{ matrix.arch }}' "$WORKFLOW" >/dev/null ||
    fail 'squashfuse upload path is missing'
grep -F 'release/squashfuse_ll-musl-mimalloc-${{ matrix.arch }}' "$WORKFLOW" >/dev/null ||
    fail 'squashfuse_ll upload path is missing'
